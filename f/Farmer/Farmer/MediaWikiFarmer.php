...
class MediaWikiFarmer {
	...
	public function __construct( $params ) {
		...
		/** Other2 **/
		$this->_scriptUrl  = $params['scriptUrl'];
		$this->_styleUrl  = $params['styleUrl'];
		$this->_uploadPath  = $params['uploadPath'];
		$this->_uploadUrl  = $params['uploadUrl'];
		$this->_tmpPath  = $params['tmpPath'];
		$this->_logoPath  = $params['logoPath'];
		$this->_logoUrl  = $params['logoUrl'];
		$this->_interwikiUrl = $params['interwikiUrl'];
	...
	public function run() {
		...
		if ( is_callable( $this->_matchFunction ) ) {
            ...
            if ($wiki=='wiki') {
                ...
                $wiki = $wgFarmerSettings['defaultWiki'];
		    }
			$this->_doWiki( $wiki );
