...
class IndexFunction {
	...
	public function getTargets() {
		...
		$this->mTo = self::newFromIDs( $ids );

		return $this->mTo;
	}

	private static function newFromIDs( $ids ) {
		$services = MediaWiki\MediaWikiServices::getInstance();
		if ( method_exists( $services, 'getPageStore' ) ) {
			// MW 1.36+
			$pageRecords = $services
				->getPageStore()
			...
			foreach ( $pageRecords as $pageRecord ) {
				$ret[] = $titleFactory->castFromPageIdentity( $pageRecord );
			}
			return $ret;
		} else {
	...
	public function makeTargetList() {
		...
		foreach ( $targets as $t ) {
			$link = $linkRenderer->makeKnownLink( $t, $t->getPrefixedText() );
