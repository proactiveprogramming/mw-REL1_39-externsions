...
class OpenIDHooks {
	...
	private static function getAssociatedOpenIDsTable( $user ) {
		...
		foreach ( $openid_urls_registration as $url_reg ) {
			...
			$rows .= Xml::tags( 'tr', [],
				...
				Xml::tags( 'td',
					[],
					$linkRenderer->makeLink( $delTitle, wfMessage( 'openid-urls-delete' )->text(),
						[],
		...
		foreach ( $trusted_sites as $key => $value ) {
			...
			if ( $key !== "" ) {
				$rows .= Xml::tags( 'tr', [],
					...
					Xml::tags( 'td',
						[],
						$linkRenderer->makeLink( $deleteTrustedSiteTitle,
	...
	public static function onGetPreferences( $user, &$preferences ) {
		...
		if ( method_exists( $wgAuth, 'allowPasswordChange' ) && $wgAuth->allowPasswordChange() ) {
			$resetlink = MediaWikiServices::getInstance()->getLinkRenderer()->makeLink(
