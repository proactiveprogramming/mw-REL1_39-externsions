...
class CreateCbxThumbnailsJob extends Job {
	/**
	...
	public function __construct( $title, $params ) {
		parent::__construct( 'createCbxThumbnailsJob', $title, $params );
	}
	...
	public static function insertJobs( $upload, $mime, &$error ) {
		global $wgCbxCreateThumbnailsInJobQueue;
		if ( !$wgCbxCreateThumbnailsInJobQueue ) {
			return true;
		}
		$magic = MediaWiki\MediaWikiServices::getInstance()->getMimeAnalyzer();
		if ( !$magic->isMatchingExtension( 'cbr', $mime ) && !$magic->isMatchingExtention('cbz',$mime ) ) {
			wfDebugLog( 'thumbnails', 'not cbx?...' );
			return true; // not a cbr or cbz, abort
		}
		...
		$jobs = [];
		for ( $i = 1; $i <= $pages; $i++ ) {
			$jobs[] = new CreateCbxThumbnailsJob(
				$title,
