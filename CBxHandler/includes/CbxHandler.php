...
class CbxHandler extends ImageHandler {
	public static $messages = [
		'main' => 'cbx-file-page-warning',
		'header' => 'cbx-file-page-warning-header',
		'info' => 'cbx-file-page-warning-info',
		'footer' => 'cbx-file-page-warning-footer',
	];
	...
	public function isEnabled() {
		global $wgCbxProcessor, $wgCbxPostProcessor, $wgCbxInfo;

		if ( !isset( $wgCbxProcessor ) || !isset( $wgCbxPostProcessor ) || !isset( $wgCbxInfo ) ) {
			wfDebug( "CbxHandler is disabled, please set the following\n" );
			wfDebug( "variables in LocalSettings.php:\n" );
			wfDebug( "\$wgCbxProcessor, \$wgCbxPostProcessor, \$wgCbxInfo\n" );
			return false;
		}
		return true;
	}
	...
	public function doTransform( $image, $dstPath, $dstUrl, $params, $flags = 0 ) {
		global $wgCbxProcessor, $wgCbxPostProcessor, $wgCbxHandlerDpi, $wgCbxHandlerJpegQuality;
	 ...
	 */
	private function getCbxImage( $image, $path ) {
		if ( !$image ) {
			$cbximg = new CbxImage( $path );
		} elseif ( !isset( $image->cbxImage ) ) {
			$cbximg = $image->cbxImage = new CbxImage( $path );
		} else {
			$cbximg = $image->cbxImage;
		}
	...
	private function getMetaArray( $image ) {
		if ( isset( $image->cbxMetaArray ) ) {
			return $image->cbxMetaArray;
		}

		$metadata = $image->getMetadata();

		if ( !$this->isMetadataValid( $image, $metadata ) ) {
			wfDebug( "Cbx metadata is invalid or missing, should have been fixed in upgradeRow\n" );
			return false;
		}

		$work = new PoolCounterWorkViaCallback(
			'CbxHandler-unserialize-metadata',
			$image->getName(),
			[
				/**
				 * @suppress PhanUndeclaredProperty
				...
				'doWork' => function () use ( $image, $metadata ) {
					wfSuppressWarnings();
					$image->cbxMetaArray = unserialize( $metadata );
					wfRestoreWarnings();
				},
			]
		);
	...
	public function getImageSize( $image, $path ) {
		return $this->getCbxImage( $image, $path )->getImageSize();
	}
	...
	public function getThumbType( $ext, $mime, $params = null ) {
		global $wgCbxOutputExtension;
		static $mime;

		if ( !isset( $mime ) ) {
			$magic = MediaWiki\MediaWikiServices::getInstance()->getMimeAnalyzer();
			$mime = $magic->guessTypesForExtension( $wgCbxOutputExtension );
		}
		return [ $wgCbxOutputExtension, $mime ];
	}
	...
	public function getMetadata( $image, $path ) {
		return serialize( $this->getCbxImage( $image, $path )->retrieveMetaData() );
	}
	...
	protected function getDimensionInfo( File $file ) {
		...
		return $cache->getWithSetCallback(
			$cache->makeKey( 'file-cbx', 'dimensions', $file->getSha1() ),
			...
			function () use ( $file ) {
				...
				for ( $i = 1; $i <= $count; $i++ ) {
					$dimsByPage[$i] = CbxImage::getPageSize( $data, $i );
				}
	...
	public static function registerWarningModule( &$resourceLoader ) {
		$resourceLoader->register( 'cbxhandler.messages', [
