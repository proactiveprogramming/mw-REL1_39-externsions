...
class SpecialOpenIDLogin extends SpecialOpenID {
	...
	function execute( $par ) {
		global $wgOpenIDForcedProvider, $wgOpenIDProviders, $wgOut;
		...
		default: # Main entry point
			if ( $this->getRequest()->getText( 'returnto' ) ) {
				$this->setReturnTo( $this->getRequest()->getText( 'returnto' ), $this->getRequest()->getVal( 'returntoquery' ) );
			}
		...
		$wgOut->addHTML(
			Html::rawElement( 'form',
				...
				Html::Hidden( 'openidProviderSelectionLoginToken',
					$this->getRequest()->getSession()->getToken( '', 'login' )->toString() )
			)
		);
	...
	function chooseName() {
		...
		$token = $this->getRequest()->getSession()->getToken( '', 'login' )->toString();
		if ( $token != $this->getRequest()->getVal( 'openidChooseNameBeforeLoginToken' ) ) {
		...
		$choice = $this->getRequest()->getText( 'wpNameChoice' );
		$nameValue = $this->getRequest()->getText( 'wpNameValue' );

		if ( $choice == 'existing' ) {
			$user = $this->attachUser( $openid, $sreg,
				$this->getRequest()->getText( 'wpExistingName' ),
				$this->getRequest()->getText( 'wpExistingPassword' )
			);
			...
			foreach ( [ 'fullname', 'nickname', 'email', 'language' ] as $option ) {
				if ( $this->getRequest()->getCheck( 'wpUpdateUserInfo' . $option ) ) {
					$force[] = $option;
				}
			}
	...
	function updateUser( $user, $sreg, $ax, $force = false ) {
		...
		if ( $this->updateOption( 'nickname', $user, $force ) ) {
			if ( array_key_exists( 'nickname', $sreg ) && $sreg['nickname'] != $this->userOptionsManager->getOption( $user, 'nickname' ) ) {
				$this->userOptionsManager->setOption( $user, 'nickname', $sreg['nickname'] );
			}
		}
		...
		if ( $this->updateOption( 'language', $user, $force ) ) {
			if ( array_key_exists( 'language', $sreg ) ) {
				...
				$this->userOptionsManager->setOption( $user, 'language', $sreg['language'] );
			}
		}
		...
		if ( $this->updateOption( 'timezone', $user, $force ) ) {
			if ( array_key_exists( 'timezone', $sreg ) ) {
				...
				# $this->userOptionsManager->setOption('timecorrection', $offset);
			}
		}
	...
	private function updateOption( $option, User $user, $force ) {
		$ret = ( $force === true
			...
			|| $this->userOptionsManager->getOption( $user, 'openid-userinfo-update-on-login-' . $option ) );
