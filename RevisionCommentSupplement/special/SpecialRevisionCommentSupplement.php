...
class SpecialRevisionCommentSupplement extends SpecialPage {
		...
		if ( $row === false ) {
			...
			if ( $row->rcs_supplement && ( $row->rcs_supplement != '*' ) ) {
				if ( method_exists( MediaWikiServices::class, 'getCommentFormatter' ) ) {
					// MW 1.38+
					$supplement = MediaWikiServices::getInstance()->getCommentFormatter()->format( $row->rcs_supplement );
				} else {
	...
	function showRevisionHistoryLine( $revId ) {
		...
		if ( method_exists( MediaWikiServices::class, 'getCommentFormatter' ) ) {
			// MW 1.38+
			$s .= MediaWikiServices::getInstance()->getCommentFormatter()
				->formatRevision( $rev, $this->getUser(), false, true );
		} else {
