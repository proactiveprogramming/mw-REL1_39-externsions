...
use File;
use Html;
use MediaWiki\Extension\ImageMap\ImageMap;
...
class GraphViz {
	...
	public static function getUser() {
		...
		if ( $pmgr->userHasAllRights( $user, ...$requiredRights ) ) {
			return $user;
		}
	...
	protected static function render( $input, $args, Parser $parser, PPFrame $frame ) {
		...
		if ( method_exists( $parserOutput, 'setPageProperty' ) ) {
			// MW 1.38
			$parserOutput->setPageProperty( 'graphviz', true );
		} else {
			$parserOutput->setProperty( 'graphviz', true );
		}
	...
	protected static function sanitizeDotInput( &$input, &$errorText ) {
		...
		$input = preg_replace_callback( $pattern, [ self::class, 'fixImageName' ], $input, $limit, $count );
		...
		$input = preg_replace_callback(
			self::DOT_IMG_PATTERN, [ self::class, 'fixImgSrc' ], $input, $limit, $count
		);
