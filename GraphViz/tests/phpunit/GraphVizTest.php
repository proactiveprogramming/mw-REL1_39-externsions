 ...
 * @group Database
 */
class GraphVizTest extends MediaWikiIntegrationTestCase {
	...
	public function testCreateGraph() {
		...
		$testTitle = $this->insertPage( 'GraphViz test 1', $dotSource, null, $user );
		if ( $wikiPageFactory !== null ) {
			// MW 1.36+
			$testPage = $wikiPageFactory->newFromTitle( $testTitle['title'] );
		} else {
		...
		$testTitle2 = $this->insertPage( 'GraphViz test 2', $dotSource, null, $parserOptions2->getUser() );
		if ( $wikiPageFactory !== null ) {
			// MW 1.36+
			$testPage2 = $wikiPageFactory->newFromTitle( $testTitle2['title'] );
		} else {
		...
		$testTitle3 = $this->insertPage( 'GraphViz test 3', $dotSource3, null, $user );
		if ( $wikiPageFactory !== null ) {
			// MW 1.36+
			$testPage3 = $wikiPageFactory->newFromTitle( $testTitle3['title'] );
		} else {
