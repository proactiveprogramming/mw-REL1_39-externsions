...
class DBTools {
	...
	public static function safe_insert_assoc( $my_table, $my_keyfield, $my_key, $my_array ) {
		...
		$exists = [];
		if ( $safe_key != "" ) {
			$exists = self::doMultirowQuery( "SELECT $safe_keyfield FROM $safe_table WHERE $safe_keyfield=$safe_key" );
		}
		if ( count( $exists ) > 0 ) {
			self::mysql_update_assoc( $safe_table, $safe_array, "WHERE $safe_keyfield='$safe_key'" );
		} else {
			self::mysql_insert_assoc( $safe_table, $safe_array );
		}
	}
	...
	public static function unsafe_insert_assoc( $table, $keyfield, $key, $array ) {
		$exists = [];
		if ( $key != "" ) {
			$exists = self::doMultirowQuery( "SELECT $keyfield FROM $table WHERE $keyfield=$key" );
		}
		if ( count( $exists ) > 0 ) {
			self::mysql_update_assoc( $table, $array, "WHERE $keyfield='$key'" );
		} else {
			self::mysql_insert_assoc( $table, $array );
		}
	}
