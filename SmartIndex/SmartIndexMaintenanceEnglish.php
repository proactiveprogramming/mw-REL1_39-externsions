...
class SmartIndexMaintenance extends SpecialPage {
	...
	public function execute ($par) {
		...
		$wgOut->setPageTitle('Smart Index Maintenance');
		...
		if ($wgRequest->getCheck('updateindex')) {
			...
			if ($this->createIndexTable()) {
				$this->getWordData();
				$wgOut->addWikiText("Index table successfully created. " . '<br>');
			} else {
				$wgOut->addWikiText("Index table update failed." . '<br>');
			}
		}
		...
		if($wgRequest->getCheck('updatestop')) {
			...
			if($this->createStopWordTable($stopWordPage)) {
				if ($this->getStopWords($stopWordPage)) {
					$wgOut->addWikiText("Stop words successfully updated." . '<br>');
				} else {
					$wgOut->addWikiText("Unable to access the Wiki page with the entered name. Please be sure this page exists in this wiki." . '<br>');
				}
			} else {
				$wgOut->addWikiText("Unable to create stop word table in the database." . '<br>');
			}
		}
		...
		if ($wgRequest->getCheck('clearstop')) {
			...
			$wgOut->addWikiText("Stop words cleared." . '<br>');
		}
		...
		$updateWordsForm .= XML::inputLabel("Stop Word Page: ", 'stopwords', 'stopwords', 40) . '&#160;';
		$updateWordsForm .= XML::submitButton("Update Stop Words", array('name' => 'updatestop')) . '&#160;';
		$updateWordsForm .= XML::submitButton("Clear Stop Words", array('name' => 'clearstop')) . '<br />';
